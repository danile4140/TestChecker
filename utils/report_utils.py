...
def report_detail_sheet(result, excel):
    ...
    title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 16, 'valign': 'vcenter', 'fg_color': '#fff2cc'})
    ...
    sub_title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 14, 'valign': 'vcenter', 'fg_color': '#d9ead3', 'border': 1})
...
def report_outline_sheet(private_results, excel):
    ...
    # title
    title_style = excel.add_format(
        {'bold': True, 'align': 'center', 'font_size': 18, 'valign': 'vcenter', 'fg_color': '#b6d7a8'})
    ouline_sheet.merge_range('A1:M3', '手游Appstore上线预审核报告（%s）' % time.strftime("%Y-%m-%d", time.localtime()),
                             title_style)
