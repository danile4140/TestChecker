...
def log_invalid(request, ext_text):
    log = '%s - - [%s] %s %s %s' % (
        request.remote_addr, DateUtil.now_datetime(), request.method, request.path, ext_text)
...
class Log(object):
    ...
    def record(self, lmsg):
        print(lmsg)
        self.text.append(lmsg + '\n')

    def error_record(self, lmsg):
        self.errormsg.append(lmsg + '\n')

    def write_log(self):
        with open(self.logpath, 'a+') as logfileobj:
            logfileobj.writelines(self.text)
            logfileobj.writelines(self.errormsg)
