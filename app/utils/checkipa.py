...
class IntegrityCheck(object):
    ...
    def detect_errors(self):
        ...
        if self.errors['Internalversion']:
            results.append({'label': 'InternalVersion',
                            'description': 'app Internalversion is not exist'})

        if self.errors['version']:
            results.append({'label': 'Version',
                            'description': 'app version is not exist'})

            # if self.errors['UIDeviceFamily']:
            # results.append({'label': 'UIDeviceFamily',
            # 'description': 'app UIDeviceFamily is not exist'})
    ...
    def output_report(self):
        ...
        if 'CFBundleVersion' in self.data:
            txt.append('  InternalVersion: %s' % (self.data['CFBundleVersion']))

        if 'UIDeviceFamily' in self.data:
            txt.append('  UIDeviceFamily: %s' % (self.data['UIDeviceFamily']))

        if 'UIFileSharingEnabled' in self.data:
            txt.append('  UIFileSharingEnabled: %s' % (self.data['UIFileSharingEnabled']))

        if 'CFBundleShortVersionString' in self.data:
            txt.append('  Version: %s' % (self.data['CFBundleShortVersionString']))
    ...
    def process_checks(self):
        ...
        self.check_Internalversion()
        self.check_UIDeviceFamily()
        self.check_UIFileSharingEnabled()
        self.check_NSAllowsArbitraryLoads()
        self.check_UIBackgroundModes()
        self.check_UIRequiredDeviceCapabilities()
        self.check_version()
        self.check_appid()
        self.check_push()
    ...
    def check_Internalversion(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['Internalversion'] = True
        else:
            self.errors['Internalversion'] = True

    def check_version(self):
        if 'CFBundleShortVersionString' in self.data:
            if self.data['CFBundleShortVersionString'].strip() == "":
                self.errors['version'] = True
        else:
            self.errors['version'] = True

    def check_UIDeviceFamily(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['Internalversion'] = True
        else:
    ...
    def check_UIFileSharingEnabled(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['CFBundleVersion'] = True
        else:
            self.errors['CFBundleVersion'] = True

    def check_UIBackgroundModes(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['CFBundleVersion'] = True
        else:
    ...
    def check_UIRequiredDeviceCapabilities(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['CFBundleVersion'] = True
        else:
    ...
    def check_NSAllowsArbitraryLoads(self):
        if 'CFBundleVersion' in self.data:
            if self.data['CFBundleVersion'].strip() == "":
                self.errors['Internalversion'] = True
        else:
...
class ParseIPA(object):
    ...
    def version(self):
        if 'CFBundleShortVersionString' in self.info_plist_data:
            return re.findall('[\d\.]*', self.info_plist_data['CFBundleShortVersionString'])[0]
        return ''

    def UIDeviceFamily(self):
        if 'UIDeviceFamily' in self.info_plist_data and \
                        len(self.info_plist_data['UIDeviceFamily']) > 1:
            return True
        else:
            return False

    def UIFileSharingEnabled(self):
        if 'UIFileSharingEnabled' in self.info_plist_data:
            return self.plist_info_list['UIFileSharingEnabled']
        else:
            return False

    def UIBackgroundModes(self):
        if 'UIBackgroundModes' in self.info_plist_data:
            return self.info_plist_data["UIBackgroundModes"]
        else:
            return False

    def UIRequiredDeviceCapabilities(self):
        if 'UIRequiredDeviceCapabilities' in self.info_plist_data:
            return self.info_plist_data["UIRequiredDeviceCapabilities"]
        else:
            return False

    def NSAllowsArbitraryLoads(self):
        if 'NSAppTransportSecurity' in self.info_plist_data and \
                        'NSAllowsArbitraryLoads' in self.info_plist_data["NSAppTransportSecurity"]:
            return self.info_plist_data["NSAppTransportSecurity"]['NSAllowsArbitraryLoads']
        else:
            return False
...
def process_ipa(ipa_filename):
    ...
    check_result['Internalversion'] = parse.Internalversion()
    ...
    check_result['UIDeviceFamily'] = parse.UIDeviceFamily()
    check_result['UIFileSharingEnabled'] = parse.UIFileSharingEnabled()
    check_result['NSAllowsArbitraryLoads'] = parse.NSAllowsArbitraryLoads()
    check_result['UIBackgroundModes'] = parse.UIBackgroundModes()
    check_result['UIRequiredDeviceCapabilities'] = parse.UIRequiredDeviceCapabilities()
